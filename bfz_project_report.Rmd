...
analyzed_white <- ukb_subset(bd_BFZ_df, "White")
analyzed_white_bp <- ukb_subset(bd_BFZ_df, "White", "sbp")
analyzed_white_glucose <- ukb_subset(bd_BFZ_df, "White", "glucose")
analyzed_white_urate <- ukb_subset(bd_BFZ_df, "White", "urate")

analyzed_black <- ukb_subset(bd_BFZ_df, "Black")
analyzed_black_bp <- ukb_subset(bd_BFZ_df, "Black", "sbp")
analyzed_black_glucose <- ukb_subset(bd_BFZ_df, "Black", "glucose")
analyzed_black_urate <- ukb_subset(bd_BFZ_df, "Black", "urate")

analyzed_asian <- ukb_subset(bd_BFZ_df, "Asian")
analyzed_asian_bp <- ukb_subset(bd_BFZ_df, "Asian", "sbp")
analyzed_asian_glucose <- ukb_subset(bd_BFZ_df, "Asian", "glucose")
analyzed_asian_urate <- ukb_subset(bd_BFZ_df, "Asian", "urate")
```
...
for(i in seq_along(races)){
  ...
  bd_BFZ_race <- bd_BFZ_race %>%
    select(new_covariates) 
...
bd_imp <- bd_BFZ_df %>% 
  filter(race == race_BFZ) %>%
  select(-f.eid, -I, -date, -race)   # 28,160 whites
...
bd_imp <- as_tibble(complete(tempData, 1, include = FALSE)) %>%
  ...
  as_tibble() %>%
  select(-`(Intercept)`) 
colnames(bd_imp)[1:13] <- c("age1", "age2", "age3", "age4", "age5", "age6",
                            "sexMale", "bmi1", "bmi2", "bmi3", "bmi4", "bmi5",
                            "bmi6")
...
write.table(sample_file, file = paste("sample_", race_BFZ, ".sample", sep = ""), 
            sep=" ", row.names=FALSE)
# Manually edit the second row of the sample file to include D (discrete covariate),
# C (continuous covariate), B (binary outcome) and P (continuous outcome).
# 0 0 0 C C C C C C D C C C C C C D D D D C D D C C C C C C C C C C P P P P
...
for (race in seq_along(races)){
  ...
  bd_imp <- bd_BFZ_df %>% 
    filter(race == race_BFZ) %>%
    select(-f.eid, -I, -date, -race)          # 791 blacks
  md.pattern(bd_imp)   # Extent of missing data
  tempData <- mice(bd_imp, m = 1, maxit = 10, seed = 7) # Impute missing data 
  #stripplot(tempData, pch = 20, cex = 1.2) # Plot of imputed dataset
  bd_imp <- as_tibble(complete(tempData, 1, include = FALSE)) %>%
    select(age:C10) %>%
    model.matrix(~ age + sex + bmi + smoking_status + alcohol_status + 
                   townsend_index + pa + C1 + C2 + C3 + C4 + C5 + C6 + C7 + C8 + 
                   C9 + C10, .
    ) %>%
    as_tibble() %>%
    select(-`(Intercept)`) # Get the imputed dataset, remove outcomes, and change 
  # to matrix format
  categorical <- c("sexMale", "smoking_statusPrevious", "smoking_statusCurrent",
                   "alcohol_statusPrevious", "alcohol_statusCurrent", "palight",
                   "paheavy")
...
# C (continuous covariate), B (binary outcome) and P (continuous outcome).
#0 0 0 C D C D D D D C D D C C C C C C C C C C P P P P
```
